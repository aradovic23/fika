...
export const drinksRouter = createTRPCRouter({
    ...
    deleteFile: publicProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            const file = await ctx.prisma.picture.delete({
                where: {
                    id: input.fileId,
                },
            });

            const product = await ctx.prisma.drink.update({
                where: {
                    id: input.productId,
                    pictureId: input.fileId,
                },
                data: {
                    image: null,
                    pictureId: null,
            ...
            if (!file || !product) throw new TRPCError({ code: 'NOT_FOUND' });
        }),
